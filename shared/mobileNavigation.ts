...
export type MobilePlannerDestination =
  (typeof mobilePlannerDestinations)[number]["id"];

export const mobilePrimaryPlannerDestinations = mobilePlannerDestinations.slice(
  0,
  4
);
...
export function hasCompletePhoneNavigationPattern(
  ...
  return (
    hasCompleteMobilePlannerNavigation([...primary, ...more]) &&
    primary.length <= 4 &&
    more.length >= 1
  );
}
