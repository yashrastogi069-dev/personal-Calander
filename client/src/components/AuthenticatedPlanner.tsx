...
const AuthenticatedAccountContext = createContext<AuthenticatedAccount | null>(
  null
);
...
export function useAuthenticatedAccount() {
  ...
  if (!account)
    throw new Error(
      "useAuthenticatedAccount must be used inside AuthenticatedPlanner"
    );
  return account;
}
...
export function AuthenticatedPlanner({ children }: { children: ReactNode }) {
  const auth = useAuth();
  const workspace = trpc.auth.workspace.useQuery(undefined, {
    ...
    () =>
      workspace.data
        ? {
            ...
            timezone: safeTimeZone(workspace.data.timezone),
          }
        : null,
    [workspace.data]
  );
  ...
  if (workspace.error)
    return (
      <main className="p-8" role="alert">
        ...
        <Button onClick={() => void workspace.refetch()}>Try again</Button>
        {signOut}
      </main>
    );
  ...
  if (!scope)
    return (
      <main className="p-8">
        ...
        <p>
          Sign-in succeeded. The deployment owner must link your existing
          planner to this account before you can continue.
        </p>
        <Button onClick={() => void workspace.refetch()}>Check again</Button>
        {signOut}
      </main>
    );
  return (
    <WorkspaceContext.Provider key={auth.user.id} value={scope}>
      <PlannerSyncScopeContext.Provider
        value={{
          accountId: auth.user.authUserId!,
