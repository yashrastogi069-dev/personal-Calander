...
import { localDateInTimezone, shiftLocalDate } from "@/lib/workspace";
...
export default function CalendarExecution() {
  ...
  return <main className="calendar-execution-page"><header className="calendar-execution-page-top"><Link href="/?surface=today" className="calendar-execution-back"><ArrowLeft size={16} /> Daily desk</Link><div className="calendar-page-title"><span className="calendar-mobile-identity"><span className="calendar-mobile-mark" aria-hidden="true"><span /></span><span><b>Personal</b><strong>Calendar</strong></span></span><span className="eyebrow">Calendar</span><h1>Execution calendar</h1></div><Link href="/?surface=tasks"><Button type="button" variant="outline" className="calendar-execution-open-tasks">Open Tasks</Button></Link></header><CalendarExecutionWorkspace scope={scope} snapshot={snapshotQuery.data} today={today} rolloverPreview={rolloverPreview.data} rolloverLoading={rolloverPreview.isLoading} rolloverPending={applyRollover.isPending} onApplyMorningRollover={applyMorningRollover} onOpenTasks={() => navigate("/?surface=tasks")} onCreateTask={() => navigate("/?surface=tasks&create=task")} onComplete={completeTask} onUnreserve={unreserveTask} /></main>;
}
