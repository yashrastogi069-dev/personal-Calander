...
import { goalHealthTriageCount, goalHealthTriageFilters, matchesGoalHealthTriage, type GoalHealthTriageFilter } from "@shared/goalHealthTriage";
...
const FocusWorkspace = lazy(() => import("@/features/focus/FocusWorkspace").then(module => ({ default: module.FocusWorkspace })));
const NaturalLanguageCaptureWorkspace = lazy(() => import("@/features/capture/NaturalLanguageCaptureWorkspace").then(module => ({ default: module.NaturalLanguageCaptureWorkspace })));
const WorkspaceSearchWorkspace = lazy(() => import("@/features/search/WorkspaceSearchWorkspace").then(module => ({ default: module.WorkspaceSearchWorkspace })));
const PlanningInsightsWorkspace = lazy(() => import("@/features/insights/PlanningInsightsWorkspace").then(module => ({ default: module.PlanningInsightsWorkspace })));
const CalendarIntegrationWorkspace = lazy(() => import("@/features/integrations/CalendarIntegrationWorkspace").then(module => ({ default: module.CalendarIntegrationWorkspace })));
const ProjectExecutionWorkspace = lazy(() => import("@/features/projects/ProjectExecutionWorkspace").then(module => ({ default: module.ProjectExecutionWorkspace })));
const HabitDisciplineWorkspace = lazy(() => import("@/features/habits/HabitDisciplineWorkspace").then(module => ({ default: module.HabitDisciplineWorkspace })));
const PlanWorkspace = lazy(() => import("@/features/planning/PlanWorkspace").then(module => ({ default: module.PlanWorkspace })));

function DestinationLoading({ label }: { label: string }) {
  return <section className="destination-loading" aria-live="polite" aria-busy="true"><div className="destination-loading-heading"><Skeleton className="h-3 w-24" /><Skeleton className="h-8 w-48" /></div><div className="destination-loading-grid"><Skeleton className="h-28 rounded-[1.25rem]" /><Skeleton className="h-28 rounded-[1.25rem]" /><Skeleton className="h-52 rounded-[1.25rem]" /></div><p className="sr-only">Opening {label}…</p></section>;
}
...
function TaskRow({ task, categoryColor, parentTitle, childCount = 0, projects, onToggle, onSchedule, onQuickReschedule, onArchive }: { task: any; categoryColor?: string; parentTitle?: string; childCount?: number; projects: any[]; onToggle: (task: any) => void | Promise<unknown>; onSchedule?: (task: any, date: string) => void; onQuickReschedule?: (task: any, amount: number) => void; onArchive?: (task: any) => void }) {
  ...
  const clearPointerInteraction = () => { pointerStart.current = null; if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } };
  const recordPointerStart = (event: React.PointerEvent<HTMLElement>) => {
    clearPointerInteraction();
    pointerStart.current = { x: event.clientX, y: event.clientY, pointerType: event.pointerType };
    if (event.pointerType === "touch") longPressTimer.current = setTimeout(() => { setSwipeAction(null); setEditorOpen(true); longPressTimer.current = null; }, 520);
  };
  const resolveSwipe = (event: React.PointerEvent<HTMLElement>) => {
    const start = pointerStart.current;
    clearPointerInteraction();
    if (!start) return;
    const gesture = resolveMobileTaskGesture({ pointerType: start.pointerType, startX: start.x, startY: start.y, endX: event.clientX, endY: event.clientY, completed });
    if (gesture === "complete") { setSwipeAction(null); onToggle(task); return; }
    if (gesture === "reveal_archive" && onArchive) setSwipeAction("archive");
  };
  return <><article className={cn("task-row", completed && "is-complete", onQuickReschedule && "is-reschedulable", swipeAction && "has-swipe-action")} draggable={!completed && Boolean(onSchedule)} onDragStart={event => event.dataTransfer.setData("text/plain", task.id)} onPointerDown={recordPointerStart} onPointerUp={resolveSwipe} onPointerCancel={clearPointerInteraction}>
    {swipeAction ? <div className="task-swipe-action" role="status"><span>Ready to archive</span><button type="button" onClick={() => { onArchive?.(task); setSwipeAction(null); }} aria-label={`Archive ${task.title}`}>Archive</button><button type="button" onClick={() => setSwipeAction(null)}>Cancel</button></div> : null}<TaskCheck checked={completed} label={task.title} onClick={() => onToggle(task)} />
...
function TaskBoard({ tasks, categories, projects, onToggle, onMove, onCompose, onArchiveCompleted, onArchive, isSearching }: { tasks: any[]; categories: any[]; projects: any[]; onToggle: (task: any) => Promise<string | null>; onMove: (task: any, lane: TaskBoardLaneId) => Promise<string | null>; onCompose: () => void; onArchiveCompleted: (tasks: any[]) => Promise<boolean>; onArchive: (task: any) => void; isSearching: boolean }) {
  ...
  const move = (task: any, lane: TaskBoardLaneId) => {
    if (laneForTaskState(task.state) === lane) { setActionError(null); setMoveAnnouncement(`${task.title} is already in ${taskBoardLanes.find(item => item.id === lane)?.label}.`); return; }
    setActionError(null);
    setMoveAnnouncement(`Moving ${task.title} to ${taskBoardLanes.find(item => item.id === lane)?.label}.`);
    void onMove(task, lane).then(message => { if (message) setActionError(message); });
  };
  ...
  return <section className="task-board" aria-labelledby="task-board-heading">
    ...
    {actionError ? <p className="form-error task-board-error" role="alert">{actionError} The task remains unchanged.</p> : null}
  ...
  return (
    <section className="focus-panel" data-entry-stage={entryStage} aria-labelledby="focus-heading">
      ...
      <div className="focus-list">
        {tasks.length ? tasks.map(task => <TaskRow key={task.id} task={task} categoryColor={categoryColors.get(task.categoryId)} projects={projects} onToggle={onToggle} onQuickReschedule={quickReschedule} onArchive={onArchive} />) : <EmptyState title="Begin with one honest commitment" detail="Capture a task, then decide whether it belongs in today." action={onCompose} />}
      </div>
...
function OfflineCaptureIndicator() {
  ...
  useEffect(() => {
    ...
    const markOnline = () => { setIsOnline(true); refresh(); };
    const markOffline = () => setIsOnline(false);
    window.addEventListener(offlineCaptureChangedEvent, refresh);
    window.addEventListener("online", markOnline);
    window.addEventListener("offline", markOffline);
    return () => { window.removeEventListener(offlineCaptureChangedEvent, refresh); window.removeEventListener("online", markOnline); window.removeEventListener("offline", markOffline); };
  }, [scope.workspaceId]);
  if (isOnline && !queued) return null;
  return <p className="offline-capture-indicator" role="status">{isOnline ? `${queued} capture${queued === 1 ? "" : "s"} saved on this device and waiting for a connection.` : `Offline mode. New quick captures stay on this device${queued ? ` · ${queued} waiting to sync` : ""}.`}</p>;
}
...
function HorizonCompass({ goals }: { goals: any[] }) {
  ...
  const [triageFilter, setTriageFilter] = useState<GoalHealthTriageFilter>("all");
  ...
  const triageCounts = Object.fromEntries(goalHealthTriageFilters.map(filter => [filter, filter === "all" ? activeGoals.length : goalHealthTriageCount(triageItems, filter)])) as Record<GoalHealthTriageFilter, number>;
  const triagedGoals = activeGoals.filter(goal => matchesGoalHealthTriage(healthByGoal.get(goal.id), triageFilter));
  const visibleGoals = showAllGoals ? triagedGoals : triagedGoals.slice(0, 6);
  const triageMeta: Record<GoalHealthTriageFilter, { label: string; detail: string }> = {
    all: { label: "All goals", detail: "Every active outcome remains visible." },
    attention: { label: "Needs attention", detail: "Overdue, behind pace, or due for review." },
    execution: { label: "Needs work", detail: "No linked active project, task, or habit yet." },
    milestones: { label: "Needs milestone", detail: "A dated goal without milestone evidence." },
  };
...
export default function Home() {
  ...
  const selectSurface = (nextSurface: Surface) => { if (nextSurface === "calendar" && typeof window !== "undefined") { window.location.assign("/calendar"); return; } setSurface(nextSurface); setMobileMoreOpen(false); if (typeof window !== "undefined") { const url = new URL(window.location.href); url.searchParams.set("surface", nextSurface); window.history.replaceState(null, "", url); } };
  ...
  const moveTaskToLane = async (task: any, lane: TaskBoardLaneId): Promise<string | null> => {
    ...
    try {
      const updated = await updateTask.mutateAsync({ ...scope, id: task.id, expectedVersion: task.version, patch: { state: nextState } });
  ...
  const toggleTask = (task: any) => moveTaskToLane(task, task.state === "completed" ? "todo" : "completed");
  const archiveTaskFromPhone = (task: any) => {
    if (task.state === "archived") return;
    updateTask.mutate({ ...scope, id: task.id, expectedVersion: task.version, patch: { state: "archived" } }, {
      onSuccess: () => { toast.success(`${task.title} archived. Restore it from Archived work.`); invalidatePlan(); },
      onError: error => toast.error(error.message || "This task could not be archived. It remains unchanged."),
    });
  };
