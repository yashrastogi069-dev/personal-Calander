...
function TaskRow({ task, categoryColor, onToggle, onSchedule }: { task: any; categoryColor?: string; onToggle: (task: any) => void; onSchedule?: (task: any, date: string) => void }) {
  ...
  return <><article className={cn("task-row", completed && "is-complete")} draggable={!completed && Boolean(onSchedule)} onDragStart={event => event.dataTransfer.setData("text/plain", task.id)}>
    ...
    <button className="icon-quiet" aria-label={`Edit ${task.title}`} onClick={() => setEditorOpen(true)}><MoreHorizontal size={17} /></button>
...
function HabitPanel({ habits, checkIns, today, streaks = [], onCheckIn, onCompose }: { habits: any[]; checkIns: any[]; today: string; streaks?: { habitId: string; streak: number }[]; onCheckIn: (habitId: string, state: "completed" | "skipped") => void; onCompose: () => void }) {
...
export default function Home() {
  ...
  const habitCheckIn = trpc.planner.habit.checkIn.useMutation({ onSuccess: () => { utils.planner.workspace.snapshot.invalidate(); utils.planner.dashboard.invalidate(); } });
