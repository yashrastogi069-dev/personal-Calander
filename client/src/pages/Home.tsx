...
function HabitCalendarTracker({ habits, checkIns, today, onCheckIn, onClearCheckIn, pending }: { habits: any[]; checkIns: any[]; today: string; onCheckIn: (habitId: string, localDate: string, state: "completed" | "skipped") => void; onClearCheckIn: (habitId: string, localDate: string) => void; pending: boolean }) {
...
function Composer({ open, kind, categories, onOpenChange, onKindChange, onCreate, onManageCategories }: { open: boolean; kind: ComposerKind; categories: any[]; onOpenChange: (open: boolean) => void; onKindChange: (kind: ComposerKind) => void; onCreate: (values: { title: string; categoryId: string | null; dueLocalDate: string | null; estimateMinutes: number | null; recurrenceRule: Record<string, unknown> | null; recurrenceUntilLocalDate: string | null; habitFrequency: "daily" | "days_of_week" | "interval" | null; habitSchedule: Record<string, unknown> | null }) => void; onManageCategories: () => void }) {
...
export default function Home() {
  ...
  const createFromComposer = (values: { title: string; categoryId: string | null; dueLocalDate: string | null; estimateMinutes: number | null; recurrenceRule: Record<string, unknown> | null; recurrenceUntilLocalDate: string | null; habitFrequency: "daily" | "days_of_week" | "interval" | null; habitSchedule: Record<string, unknown> | null }) => {
    if (composerKind === "task") createTask.mutate({ ...scope, title: values.title, categoryId: values.categoryId, dueLocalDate: values.dueLocalDate, estimateMinutes: values.estimateMinutes, state: "not_started", priority: "medium", horizon: "weekly", sortOrder: 0, recurrenceRule: values.recurrenceRule, recurrenceAnchor: values.recurrenceRule ? "scheduled" : null, recurrenceUntilLocalDate: values.recurrenceUntilLocalDate });
    ...
    if (composerKind === "habit") createHabit.mutate({ ...scope, name: values.title, categoryId: values.categoryId, color: "#C6F06A", frequency: values.habitFrequency ?? "daily", schedule: values.habitSchedule ?? { cadence: "daily" } });
