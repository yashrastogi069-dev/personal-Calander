...
import { laneForTaskState, stateForTaskLane, taskBoardLanes, type TaskBoardLaneId } from "@shared/taskBoard";
import {
...
function TaskBoard({ tasks, categories, onToggle, onMove, onCompose, movingTaskId }: { tasks: any[]; categories: any[]; onToggle: (task: any) => void; onMove: (task: any, lane: TaskBoardLaneId) => void; onCompose: () => void; movingTaskId: string | null }) {
  ...
  const move = (task: any, lane: TaskBoardLaneId) => {
    if (laneForTaskState(task.state) === lane) {
      setMoveAnnouncement(`${task.title} is already in ${taskBoardLanes.find(item => item.id === lane)?.label}.`);
      return;
    }
  ...
  return <section className="task-board" aria-labelledby="task-board-heading"><div className="task-board-heading"><div><h2 id="task-board-heading">Work lanes</h2><p>Move work forward by dragging a task, or use its move control when you prefer the keyboard or touch.</p></div><span>{tasks.length} shown</span></div><p className="sr-only" id="task-board-help">Drag a task card to another lane to change its state. Each card also includes a Move to control.</p><p className="sr-only" role="status" aria-live="polite">{moveAnnouncement}</p><div className="task-lane-grid">{taskBoardLanes.map(lane => { const laneTasks = tasks.filter(task => laneForTaskState(task.state) === lane.id); return <section className={cn("task-lane", `task-lane-${lane.id}`, dropLane === lane.id && "is-drop-target")} key={lane.id} aria-labelledby={`task-lane-${lane.id}`}><header className="task-lane-header"><div><span className="task-lane-marker" aria-hidden="true" /><h3 id={`task-lane-${lane.id}`}>{lane.label}</h3></div><span aria-label={`${laneTasks.length} tasks`}>{laneTasks.length}</span></header><p className="task-lane-description">{lane.description}</p><div className="task-lane-list" role="list" aria-describedby="task-board-help" onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; if (dropLane !== lane.id) setDropLane(lane.id); }} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDropLane(null); }} onDrop={event => { event.preventDefault(); const taskId = event.dataTransfer.getData("text/plain") || draggedTaskId; const task = tasks.find(item => item.id === taskId); if (task) move(task, lane.id); setDraggedTaskId(null); setDropLane(null); }}>{laneTasks.length ? laneTasks.map(task => <div key={task.id} className={cn("task-lane-task", draggedTaskId === task.id && "is-dragging", movingTaskId === task.id && "is-moving")} role="listitem" draggable onDragStart={event => startDrag(event, task)} onDragEnd={() => { setDraggedTaskId(null); setDropLane(null); }}><span className="task-drag-handle" aria-hidden="true"><GripVertical size={15} /></span><TaskRow task={task} categoryColor={categoryColors.get(task.categoryId)} onToggle={onToggle} /><label className="task-lane-select"><span>Move to</span><select value={laneForTaskState(task.state)} onChange={event => move(task, event.target.value as TaskBoardLaneId)} disabled={movingTaskId === task.id}>{taskBoardLanes.map(option => <option key={option.id} value={option.id}>{option.label}</option>)}</select></label></div>) : <div className="task-lane-empty">{lane.id === "todo" ? <button type="button" onClick={onCompose}>Add a task to begin</button> : `No tasks are ${lane.id === "in_progress" ? "in progress" : "completed"} yet.`}</div>}</div></section>; })}</div></section>;
}
...
export default function Home() {
  ...
  const selectSurface = (nextSurface: Surface) => { setSurface(nextSurface); setMobileMoreOpen(false); };
  ...
  const moveTaskToLane = async (task: any, lane: TaskBoardLaneId) => {
    const nextState = stateForTaskLane(lane);
    if (task.state === nextState) return;
    setMovingTaskId(task.id);
    try {
      await updateTask.mutateAsync({ ...scope, id: task.id, expectedVersion: task.version, patch: { state: nextState } });
      toast.success(`${task.title} moved to ${taskBoardLanes.find(item => item.id === lane)?.label}.`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "This task could not be moved. Refresh the board and try again.");
    } finally {
