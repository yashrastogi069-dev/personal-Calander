...
function Timeline({ tasks, selectedDate, onDrop, onMoveDay, onOpenTasks, scheduleError, onRetrySchedule }: { tasks: any[]; selectedDate: string; onDrop: (id: string, localDate: string) => void; onMoveDay: (amount: number) => void; onOpenTasks: () => void; scheduleError: string | null; onRetrySchedule: () => void }) {
...
export default function Home() {
  ...
  const [taskFilter, setTaskFilter] = useState<"all" | "today" | "deadline_risk">("all");
  ...
  const focusDeadlineRisk = () => { setTaskSearch(""); setTaskFilter("deadline_risk"); selectSurface("tasks"); };
...
function CalendarMatrix({ mode, anchor, tasks, categories, today, onMoveDay, onOpenTasks }: { mode: CalendarMode; anchor: string; tasks: any[]; categories: any[]; today: string; onMoveDay: (amount: number) => void; onOpenTasks: () => void }) {
  ...
  return <div className={cn("calendar-matrix", `matrix-${mode.toLowerCase()}`)}>
    ...
    <p className="calendar-task-legend">Tasks and time blocks live here. Open Habits for the habit-specific calendar and check-in tracker.</p>
