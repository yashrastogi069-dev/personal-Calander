...
function DestinationLoading({ label }: { label: string }) {
  return <section className="destination-loading" aria-live="polite" aria-busy="true"><span /><p>Opening {label}…</p></section>;
}
...
function TaskRow({ task, categoryColor, parentTitle, childCount = 0, projects, onToggle, onSchedule, onQuickReschedule }: { task: any; categoryColor?: string; parentTitle?: string; childCount?: number; projects: any[]; onToggle: (task: any) => void | Promise<unknown>; onSchedule?: (task: any, date: string) => void; onQuickReschedule?: (task: any, amount: number) => void }) {
...
function TaskBoard({ tasks, categories, projects, onToggle, onMove, onCompose, onArchiveCompleted, isSearching }: { tasks: any[]; categories: any[]; projects: any[]; onToggle: (task: any) => Promise<string | null>; onMove: (task: any, lane: TaskBoardLaneId) => Promise<string | null>; onCompose: () => void; onArchiveCompleted: (tasks: any[]) => Promise<boolean>; isSearching: boolean }) {
...
function FocusPanel({ tasks, categories, projects, onToggle, onCompose }: { tasks: any[]; categories: any[]; projects: any[]; onToggle: (task: any) => void; onCompose: () => void }) {
  ...
  return (
    <section className="focus-panel" data-entry-stage={entryStage} aria-labelledby="focus-heading">
      ...
      <div className="focus-list">
        {tasks.length ? tasks.map(task => <TaskRow key={task.id} task={task} categoryColor={categoryColors.get(task.categoryId)} projects={projects} onToggle={onToggle} onQuickReschedule={quickReschedule} />) : <EmptyState title="Begin with one honest commitment" detail="Capture a task, then decide whether it belongs in today." action={onCompose} />}
      </div>
