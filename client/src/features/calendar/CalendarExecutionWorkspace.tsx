...
export function CalendarExecutionWorkspace({ scope, snapshot, today, rolloverPreview, rolloverLoading, rolloverPending, onApplyMorningRollover, onOpenTasks, onComplete, onUnreserve }: { scope: WorkspaceScope; snapshot: CalendarExecutionSnapshot; today: string; rolloverPreview?: { fromLocalDate: string; candidates: Array<{ id: string; expectedVersion: number; rescheduleCount: number }> }; rolloverLoading: boolean; rolloverPending: boolean; onApplyMorningRollover: () => Promise<void>; onOpenTasks: () => void; onComplete: (task: any) => Promise<string | null>; onUnreserve: (task: any) => Promise<string | null> }) {
  ...
  const removeTime = async (task: any) => {
    const message = await onUnreserve(task);
    if (message) {
      setFeedback(message);
      return;
    }
      ...
      <section className="calendar-day-grid-wrap" aria-label={`Task execution grid for ${selectedDate}`}>
        ...
        {slots.length ? <div className="calendar-day-grid" role="grid" aria-keyshortcuts="ArrowUp ArrowDown ArrowLeft ArrowRight Enter" aria-label={`Calendar time grid for ${displayLocalDate(selectedDate, scope.timezone, { month: "long", day: "numeric" })}. Use arrow keys to choose a time. Enter reserves the selected inbox task at the next free slot.`} style={{ gridTemplateRows: `repeat(${slots.length}, 30px)` }}>
          ...
          {timedTasks.map(task => {
            ...
            return <article key={task.id} className={cn("calendar-task-block", selectedTaskId === task.id && "is-selected")} draggable style={{ gridRow: `${startIndex + 1} / span ${Math.max(1, Math.ceil(roundedDuration / taskReservationGridMinutes))}`, borderColor: categoryColors.get(task.categoryId) ?? "#61c6a5" }} onClick={() => { setSelectedTaskId(task.id); setFeedback(null); }} onDragStart={event => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-personal-calendar-task", task.id); event.dataTransfer.setData("text/plain", task.id); event.dataTransfer.setData("application/x-personal-calendar-duration", String(roundedDuration)); }}><div><span className="calendar-task-dot" style={{ backgroundColor: categoryColors.get(task.categoryId) ?? "#61c6a5" }} /><b>{task.title}</b></div><span>{timeLabel(taskReservationLocalParts(new Date(task.plannedStartAt), scope.timezone).minuteOfDay)} · {roundedDuration} min</span><button type="button" className="calendar-block-complete" onClick={() => void completeTask(task)} aria-label={`Complete ${task.title}`}><Check size={14} /> Complete</button><div className="calendar-block-actions"><div className="calendar-block-resize"><button type="button" onClick={() => void reserveAt(task, taskReservationLocalParts(new Date(task.plannedStartAt), scope.timezone).minuteOfDay, roundedDuration - taskReservationGridMinutes)} disabled={roundedDuration <= taskReservationGridMinutes} aria-label={`Shorten ${task.title} by 15 minutes`}>−15</button><button type="button" onClick={() => void reserveAt(task, taskReservationLocalParts(new Date(task.plannedStartAt), scope.timezone).minuteOfDay, roundedDuration + taskReservationGridMinutes)} aria-label={`Extend ${task.title} by 15 minutes`}>+15</button></div><button type="button" className="calendar-block-remove" onClick={() => void removeTime(task)} aria-label={`Remove time for ${task.title}. The task will remain active.`}><X size={12} /> Remove time</button></div></article>;
          })}
