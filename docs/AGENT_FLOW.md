...
- Provisional production direction recorded to continue the owner's autonomous `/goal`: A as the default light architecture with exact R20 lanes, C as the selectable dark theme, comfortable default density with compact optional, and the tested short sheet transition with reduced-motion fallback. The owner may override before push or deployment. This authorizes local production styling only; it does not authorize schema migration, push, deployment, merge, feature removal, data mutation, or integrations.
