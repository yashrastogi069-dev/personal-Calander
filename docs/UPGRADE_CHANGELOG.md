...
The user has confirmed a visible manual push notification on the installed iPhone after VAPID configuration was corrected. The corrected global callback and schema registry now have local type, build, and **48-test** regression coverage, including session-independent rule activation and task-UID-gated global sweeping. The project scheduler is registered, the public r3 Vercel artifact is verified, and the user confirmed that the iPhone control now displays **Pause reminders** after enabling the cadence. The database contains the two expected enabled Auckland rules. Automatic future delivery is still not claimed before the scheduled provider call occurs and is observed.
