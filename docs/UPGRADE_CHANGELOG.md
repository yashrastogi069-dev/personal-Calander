...
The approved reminder rules are defined for `Pacific/Auckland`: daily planning at 11:00 and weekly review at Sunday 17:00. A deployed hourly authenticated callback will evaluate these wall-clock rules through the IANA timezone database, reserve a unique per-rule/per-device/local-minute delivery before sending, and treat retries, duplicate attempts, no-active-device runs, pauses, and terminal subscription expiration safely. The implementation has local daylight-saving, router, delivery, and failure-path coverage. At this changelog point, it remains a **release candidate**: production callback scheduling and actual installed-iPhone delivery must still be verified before live automated delivery is claimed.
