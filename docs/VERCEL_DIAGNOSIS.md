...
The Vercel Production variables `VITE_VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, and `VAPID_SUBJECT` were subsequently configured without recording their values in source control. A fresh production deployment served the repaired browser bundle, and the user confirmed an installed-iPhone visible manual test notification. This proves device enrollment, public-key build configuration, server signing, and a VAPID provider send; it does not itself prove scheduled cadence activation.

The original cadence mutation attempted to create end-user-owned Heartbeat tasks from an anonymous PWA session. That model did not activate the rules. Commit `ce56528` replaces it with one project-owned hourly callback, a durable `reminderSchedulers` task-ID registry, and rule-only enable/pause actions. The source was pushed to GitHub, but this session’s Vercel dashboard browser is logged out, so dashboard-level deployment status cannot currently be inspected. A follow-up non-sensitive browser marker `project-scheduler-r3` is prepared for the next Vercel build; the user must not be asked to retry cadence activation until that alias marker is observed.
