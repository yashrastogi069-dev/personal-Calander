...
The scheduled service regression now also proves that a paused rule creates no reservation or provider request, and that a terminal `410` response expires only the affected scheduled device. The full local automated suite now passes with **48 tests across 10 files**; TypeScript checking and the Vercel-targeted production build pass after the scheduler correction. The one global callback remains uncreated until the corrected version is deployed. Consequently, automatic scheduled delivery is not claimed as live evidence yet; manual installed-iPhone push delivery is confirmed separately above.
