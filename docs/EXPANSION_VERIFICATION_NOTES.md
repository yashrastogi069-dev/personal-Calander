...
## R20 first-run workflow and creator repair — local evidence

The desktop Today view now reduces an empty day to its next useful commitments: one capture action, a compact no-reservation decision, and a clear statement that planning controls appear after a task exists. The time canvas does not render empty hourly rows; it instead offers **Choose a task to reserve time**, **Open Habit tracker**, and **Keep the day open**. The direct Habit action was clicked and routed to the dedicated Habit tracker, where its completion, skip, trace, and Habit Calendar controls remained present. Daily capacity remained visible with `0 / 360 min`, clear Reserved/Deadline-only/Risk values, and a functional **Review deadline risk** handoff to the safe empty Tasks lens.

The top-bar New action was opened at desktop and now renders a compact task-first dialog with a visible Create Task action. A named disposable task was created successfully, appeared in To do, then was archived through the task editor; the active Tasks board returned to its original two records. The Project tab loads the existing active goal in its goal-link selector and names the reviewed linked-task breakdown path. A full 390×844 capture confirms the compact creator entry, readable secondary labels, progressive task canvas, direct Habit route, and capacity action remain visible without horizontal overflow. The final local regression passes TypeScript, **79 tests across 21 files**, and the Vercel-targeted production build.

GitHub `main` reached `ce277a7c5f8279d94e76f6a5602a10fb787dec25`. The first Vercel response was the expected prior client during propagation. A later cache-busted public anonymous read visibly rendered the progressive Today card, **Open Habit tracker**, Daily capacity, and **Review deadline risk**. The public document marker was read directly as **`entry-flow-r20`**. No planner, Calendar feed, device, cadence, or reminder record was changed during public verification.
