...
vi.mock("../client/src/lib/trpc", () => ({
  trpc: { auth: { workspace: { useQuery: () => state.workspace } } },
}));
vi.mock("../client/src/components/SupabaseAuthGate", () => ({
  SupabaseAuthGate: () => "SIGN_IN",
}));
import { AuthenticatedPlanner } from "../client/src/components/AuthenticatedPlanner";
const render = () =>
  renderToStaticMarkup(
    createElement(AuthenticatedPlanner, { children: "PRIVATE_PLANNER" })
  );
beforeEach(() => {
  state.auth = {
    loading: false,
...
describe("active planner authentication boundary", () => {
  ...
  it("does not invent a workspace for an authenticated but unlinked account", () => {
    state.auth = {
      ...
      user: { id: 41, authUserId: "11111111-1111-4111-8111-111111111111" },
    };
  ...
  it("mounts the planner only after the server resolves the owned workspace", () => {
    ...
    const html = render();
    expect(html).toContain("PRIVATE_PLANNER");
