...
export const plannerRouter = router({
  ...
  availability: router({
    ...
    clear: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...exception } = input;
  ...
  scheduleProposal: router({
    create: protectedProcedure.input(scope.extend({ taskId: z.string(), localDate: dateString })).mutation(async ({ input }) => {
    ...
    approve: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), taskExpectedVersion: z.number().int().positive() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...proposal } = input;
      try { return await approveScheduleProposal({ workspaceId, timezone }, proposal); } catch (error) { return plannerError(error); }
    }),
    dismiss: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...proposal } = input;
      try { return await dismissScheduleProposal({ workspaceId, timezone }, proposal); } catch (error) { return plannerError(error); }
    }),
    undo: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), taskExpectedVersion: z.number().int().positive() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...proposal } = input;
  ...
  focus: router({
    ...
    resume: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...session } = input;
      try { return await resumeFocusSession({ workspaceId, timezone }, session); } catch (error) { return plannerError(error); }
    }),
    finish: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), outcome: z.enum(["done", "continue", "adjust_estimate", "stopped"]), note: z.string().max(2000).nullable().optional(), adjustedEstimateMinutes: z.number().int().min(5).max(1440).nullable().optional(), taskExpectedVersion: z.number().int().positive().optional() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...session } = input;
      try { return await finishFocusSession({ workspaceId, timezone }, session); } catch (error) { return plannerError(error); }
    }),
  }),
  category: router({
    create: protectedProcedure.input(scope.extend({ name: z.string().trim().min(1).max(80), color: z.string().regex(/^#[0-9A-Fa-f]{6}$/), sortOrder: z.number().int().optional() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...category } = input;
      return createCategory({ workspaceId, timezone }, category);
    }),
    update: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), patch: z.object({ name: z.string().trim().min(1).max(80).optional(), color: z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(), sortOrder: z.number().int().optional() }) })).mutation(async ({ input }) => {
    ...
    delete: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive() })).mutation(async ({ input }) => {
  ...
  goal: router({
    ...
    restore: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, id, expectedVersion } = input;
  ...
  task: router({
    ...
    reserve: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), localDate: dateString, plannedStartAt: z.date(), plannedEndAt: z.date() })).mutation(async ({ input }) => {
    ...
    rolloverPreview: protectedProcedure.input(scope.extend({ fromLocalDate: dateString })).query(async ({ input }) => {
    ...
    applyRollover: protectedProcedure.input(scope.extend({ fromLocalDate: dateString, tasks: z.array(z.object({ id: z.string(), expectedVersion: z.number().int().positive() })).max(100) })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...rollover } = input;
      try { return await applyMorningRollover({ workspaceId, timezone }, rollover); } catch (error) { return plannerError(error); }
    }),
    bulkSetState: protectedProcedure.input(scope.extend({ ids: z.array(z.string()).min(1).max(100), state: lifecycle })).mutation(async ({ input }) => bulkSetTaskState(input, { ids: input.ids, state: input.state })),
    addDependency: protectedProcedure.input(scope.extend({ taskId: z.string(), dependsOnTaskId: z.string(), dependencyType: z.enum(["hard", "soft"]).default("hard") })).mutation(async ({ input }) => createTaskDependency(input, input)),
    removeDependency: protectedProcedure.input(scope.extend({ id: z.string() })).mutation(async ({ input }) => removeTaskDependency(input, input)),
  }),
  occurrence: router({
    materialize: protectedProcedure.input(scope.extend({ start: dateString, end: dateString })).mutation(async ({ input }) => materializeTaskOccurrences(input, input)),
    resolve: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), state: z.enum(["completed", "skipped", "missed", "rescheduled"]), rescheduledToLocalDate: dateString.nullable().optional(), note: z.string().max(1000).nullable().optional() })).mutation(async ({ input }) => {
  ...
  dailyCheckIn: router({
    upsert: protectedProcedure.input(scope.extend({ localDate: dateString, intention: z.string().max(3000).nullable().optional(), reflection: z.string().max(5000).nullable().optional(), energy: z.number().int().min(1).max(5).nullable().optional(), mood: z.number().int().min(1).max(5).nullable().optional() })).mutation(async ({ input }) => upsertDailyCheckIn(input, input)),
  }),
  dailyPlan: router({
    upsert: protectedProcedure.input(scope.extend({ localDate: dateString, expectedVersion: z.number().int().positive().optional(), intention: z.string().max(3000).nullable().optional(), reflection: z.string().max(5000).nullable().optional(), state: z.enum(["draft", "active", "closed", "archived"]).optional() })).mutation(async ({ input }) => {
    ...
    addItem: protectedProcedure.input(scope.extend({ dailyPlanId: z.string(), taskId: z.string() })).mutation(async ({ input }) => {
    ...
    updateItem: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), state: z.enum(["committed", "done", "rescheduled", "deferred", "wont_do", "archived"]).optional(), resolvedToLocalDate: dateString.nullable().optional(), note: z.string().max(1000).nullable().optional(), position: z.number().int().min(0).max(200).optional() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...item } = input;
      try { return await updateDailyPlanItem({ workspaceId, timezone }, item); } catch (error) { return plannerError(error); }
    }),
    moveItem: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), direction: z.union([z.literal(-1), z.literal(1)]) })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...item } = input;
      try { return await moveDailyPlanItem({ workspaceId, timezone }, item); } catch (error) { return plannerError(error); }
    }),
    resolveItem: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), taskExpectedVersion: z.number().int().positive(), state: z.enum(["done", "rescheduled", "deferred", "wont_do", "archived"]), resolvedToLocalDate: dateString.nullable().optional(), note: z.string().max(1000).nullable().optional() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...item } = input;
      try { return await resolveDailyPlanItem({ workspaceId, timezone }, item); } catch (error) { return plannerError(error); }
    }),
    close: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), reflection: z.string().max(5000).nullable().optional() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...plan } = input;
      try { return await closeDailyPlan({ workspaceId, timezone }, plan); } catch (error) { return plannerError(error); }
    }),
  }),
  weeklyObjective: router({
    create: protectedProcedure.input(scope.extend({ weekStartLocalDate: dateString, title: z.string().trim().min(1).max(280), description: z.string().max(5000).nullable().optional(), goalId: z.string().nullable().optional(), projectId: z.string().nullable().optional() })).mutation(async ({ input }) => {
    ...
    carryForward: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), nextWeekStartLocalDate: dateString })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...objective } = input;
  ...
  planningTemplate: router({
    create: protectedProcedure.input(scope.extend({ kind: z.enum(["task", "project", "daily_plan"]), name: z.string().trim().min(1).max(120), description: z.string().max(1000).nullable().optional(), payload: z.record(z.string(), z.unknown()) })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...template } = input;
      return createPlanningTemplate({ workspaceId, timezone }, template);
    }),
    update: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), patch: z.object({ name: z.string().trim().min(1).max(120).optional(), description: z.string().max(1000).nullable().optional(), payload: z.record(z.string(), z.unknown()).optional() }) })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...template } = input;
  ...
  savedView: router({
    create: protectedProcedure.input(scope.extend({ name: z.string().trim().min(1).max(120), viewType: z.enum(["tasks", "goals", "projects", "calendar", "habits"]), configuration: z.record(z.string(), z.unknown()), isPinned: z.number().int().min(0).max(1).optional() })).mutation(async ({ input }) => createSavedView(input, input)),
    update: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), name: z.string().trim().min(1).max(120).optional(), configuration: z.record(z.string(), z.unknown()).optional(), isPinned: z.number().int().min(0).max(1).optional() })).mutation(async ({ input }) => {
    ...
    delete: protectedProcedure.input(scope.extend({ id: z.string() })).mutation(async ({ input }) => deleteSavedView(input, input)),
  }),
  search: router({
  ...
  notification: router({
    devices: protectedProcedure.input(scope).query(async ({ input }) => getPushDevices(input)),
    currentDevice: protectedProcedure.input(scope.extend({ endpoint: z.string().url().max(512) })).query(async ({ input }) => getPushDeviceForEndpoint(input, input)),
    enableDevice: protectedProcedure.input(scope.extend({ subscription: z.object({ endpoint: z.string().url().max(512), keys: z.object({ p256dh: z.string().min(1).max(4096), auth: z.string().min(1).max(4096) }), deviceLabel: z.string().trim().max(120).nullable().optional(), userAgent: z.string().max(512).nullable().optional() }) })).mutation(async ({ input }) => upsertPushSubscription(input, input.subscription)),
    disableDevice: protectedProcedure.input(scope.extend({ id: z.string() })).mutation(async ({ input }) => disablePushSubscription(input, input)),
    testDevice: protectedProcedure.input(scope.extend({ subscriptionId: z.string(), origin: z.string().url().max(512).refine(value => value.startsWith("https://") || value.startsWith("http://localhost"), { message: "A secure application origin is required." }) })).mutation(async ({ input }) => sendTestPush(input, input)),
  }),
  reminder: router({
    rules: protectedProcedure.input(scope).query(async ({ input }) => getReminderRules(input)),
    activateApproved: protectedProcedure.input(scope).mutation(async ({ input }) => {
    ...
    pauseApproved: protectedProcedure.input(scope).mutation(async ({ input }) => {
  ...
  review: router({
    history: protectedProcedure.input(scope.extend({ limit: z.number().int().min(1).max(24).default(12) })).query(async ({ input }) => {
      const { workspaceId, timezone, ...history } = input;
      return getReviewHistory({ workspaceId, timezone }, history);
    }),
    start: protectedProcedure.input(scope.extend({ kind: z.enum(["daily", "weekly", "monthly", "quarterly", "yearly"]), periodStartLocalDate: dateString, periodEndLocalDate: dateString, snapshot: z.record(z.string(), z.unknown()).optional() })).mutation(async ({ input }) => startReviewSession(input, input)),
    updateChecklist: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), checklist: z.record(z.string(), z.boolean()) })).mutation(async ({ input }) => {
    ...
    complete: protectedProcedure.input(scope.extend({ id: z.string(), expectedVersion: z.number().int().positive(), reflection: z.string().max(5000).nullable().optional() })).mutation(async ({ input }) => {
      const { workspaceId, timezone, ...review } = input;
      try { return await completeReviewSession({ workspaceId, timezone }, review); } catch (error) { return plannerError(error); }
    }),
  }),
  ai: router({
    draft: protectedProcedure.input(scope.extend({ thought: z.string().trim().min(3).max(4000), todayLocalDate: dateString })).mutation(async ({ input }) => {
      try {
  ...
  dashboard: protectedProcedure.input(scope.extend({ todayLocalDate: dateString, rangeStart: dateString, rangeEnd: dateString })).query(async ({ input }) => getDashboard(input, input)),
});
